...
@mutable
case class GenericArray[T](private val data: Array[T], @ghost private var toList: List[T]) {
...
object Test {
  def test(): Unit = {
    val arr: GenericArray[Int] with arr.valid = GenericArray[Int](Array(1, 2, 3), List(1, 2, 3))
    arr(0) = 10
    assert(arr(0) == 10)
  }
  def test2(arr: GenericArray[Int] with arr.valid): Unit = {
    require(arr.size > 0)
    require(arr(0) == 1)
    assert(arr(0) == 1)
