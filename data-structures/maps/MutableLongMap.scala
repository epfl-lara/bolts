...
object MutableLongMap {
  ...
  final case class LongMapLongV(
    ...
    def update(key: Long, v: Long): Boolean = {
      ...
      if (key == -key) {
        ...
        seekEntryRes match {
          ...
          case ResMissVacant(index) => updateHelperNewKey(key, v, index)
          case ResMissingBit(index) => updateHelperNewKey(key, v, index)
          case ResOk(index) => {
          if (getCurrentListMap(0).contains(key)) {
            lemmaInListMapThenSeekEntryOrOpenFindsIt(key)
          } else {
    ...
    def updateHelperNewKey(key: Long, v: Long, index: Int): Boolean = {
      ...
      require(seekEntryOrOpenDecoupled(key)(_keys, mask) == ResMissingBit(index) || seekEntryOrOpenDecoupled(key)(_keys, mask) == ResMissVacant(index))
          if (getCurrentListMap(0).contains(key)) {
            lemmaInListMapThenSeekEntryOrOpenFindsIt(key)
            check(false)
          } else {
    ...
    def lemmaNotInListMapThenSeekEntryOrOpenFindsFreeOrNothing(k: Long): Unit = {
      ...
      val seekEntryRes = seekEntryOrOpenDecoupled(k)(_keys, mask)
      seekEntryRes match {
        case ResMissingBit(index) => inRange(index, mask) && _keys(index) == 0 && !arrayContainsKeyTailRec(_keys, k, 0)
        case ResMissVacant(index) => inRange(index, mask) && _keys(index) == Long.MinValue && !arrayContainsKeyTailRec(_keys, k, 0)
        case ResUndefined() => true
        case _ => false
      }
    })
    ...
    def lemmaSeekEntryOrOpenReturnsValidIndex(k: Long): Unit = {
      ...
      case ResMissVacant(index) => inRange(index, mask)
      case ResMissingBit(index) => inRange(index, mask)
      case _ => false
    })
  ...
  case class ResIntermediate(undefined: Boolean, index: Int, x: Int) extends SeekEntryResult
  ...
  /** Checks if i is a valid index in the Array of values
    ...
    def arrayForallSeekEntryOrOpenFound(i: Int)(implicit _keys: Array[Long], mask: Int): Boolean = {
      ...
      if (i >= _keys.length) true
        ...
        LongMapLongV.seekEntryOrOpenDecoupled(_keys(i))(_keys, mask) == ResOk(i) &&
    ...
    def seekEntryDecoupled(k: Long)(implicit _keys: Array[Long], mask: Int): SeekEntryResult = {
      ...
      intermediate match {
        case ResIntermediate(undefined, index, x) if(undefined) => ResUndefined()
        case ResIntermediate(undefined, index, x) if(!undefined) => {
          val q = _keys(index)
          if (q == k) ResOk(index)
          else if (q == 0) ResMissingBit(index)
          else {
            ...
            res match {
              case ResMissVacant(index) => ResMissingBit(index)
              case _ => res
            }
          }
        } 
    ...
    def seekEntryOrOpenDecoupled(k: Long)(implicit _keys: Array[Long], mask: Int): SeekEntryResult = {
      ...
      (
        res match {
          case ResUndefined() => true
          case ResOk(index) => _keys(index) == k
          case ResMissingBit(index) => _keys(index) == 0
          case ResMissVacant(index) => _keys(index) == Long.MinValue
          case _ => false
        }
      )
    ...
    private def seekKeyOrZeroOrLongMinValueTailRecDecoupled(x: Int, ee: Int)(implicit
      ...
      decreases(MAX_ITER - x)
      val q = _keys(ee)
      if (x >= MAX_ITER) ResIntermediate(true, ee, x)
      else if (q == k || q + q == 0) ResIntermediate(false, ee, x)
      else
        seekKeyOrZeroOrLongMinValueTailRecDecoupled(x + 1, (ee + 2 * (x + 1) * x - 3) & mask)
    }.ensuring(res =>
      (res match {
          case ResIntermediate(undefined, index, x) if(undefined) => x >= MAX_ITER
          case ResIntermediate(undefined, index, x) if(!undefined) => x < MAX_ITER && x >= 0 && (_keys(index) == k || _keys(index) == 0 || _keys(index) == Long.MinValue)
          case _ => false
        }))
    ...
    private def seekKeyOrZeroReturnVacantTailRecDecoupled(x: Int, ee: Int, vacantSpotIndex: Int)(implicit
      ...
      decreases(MAX_ITER + 1 - x)
      val q = _keys(ee)
      if (x >= MAX_ITER) ResUndefined()
      else if (q == k) ResOk(ee) 
      else if (q == 0) ResMissVacant(vacantSpotIndex) 
      ...
      res match {
        case ResUndefined() => true
        case ResOk(index) => _keys(index) == k
        case ResMissVacant(index) => index == vacantSpotIndex && _keys(index) == Long.MinValue
        case  _ => false
      }
    )
    ...
    @opaque
    @pure
    def lemmaPutLongMinValuePreservesForallSeekEntryOrOpenKey1(a: Array[Long], i: Int, j: Int)(implicit mask: Int): Unit = {
      require(validMask(mask))
      require(a.length == mask + 1)
      require(i >= 0)
      require(i < a.length)
    ...
    def lemmaPutLongMinValuePreservesForallSeekEntryOrOpenKey2(a: Array[Long], i: Int, j: Int)(implicit mask: Int): Unit = {
      ...
      if(seekKeyOrZeroOrLongMinValueTailRecDecoupled(0, toIndex(a(j), mask))(a(j), a, mask) ==
               seekKeyOrZeroOrLongMinValueTailRecDecoupled(0, toIndex(a.updated(i, Long.MinValue).apply(j), mask))(a.updated(i, Long.MinValue).apply(j), a, mask)){
        check(seekEntryOrOpenDecoupled(a(j))(a, mask) == seekEntryOrOpenDecoupled(a.updated(i, Long.MinValue).apply(j))(a, mask))
      } else {
        check(seekEntryOrOpenDecoupled(a(j))(a, mask) == seekEntryOrOpenDecoupled(a.updated(i, Long.MinValue).apply(j))(a, mask))
      }
    ...
    def lemmaPutValidKeyPreservesForallSeekEntryOrOpenKey1(a: Array[Long], i: Int, k: Long, j: Int)(implicit mask: Int): Unit = {
      ...
      require(validKeyInArray(k))
      require(!arrayContainsKeyTailRec(a, k, 0))
      require(seekEntryOrOpenDecoupled(k)(a, mask) == ResMissingBit(i) || seekEntryOrOpenDecoupled(k)(a, mask) == ResMissVacant(i))
      require(arrayForallSeekEntryOrOpenFound(0)(a, mask))
      require(arrayNoDuplicates(a, 0))
      require(seekKeyOrZeroOrLongMinValueTailRecDecoupled(0, toIndex(a(j), mask))(a(j), a, mask) ==
    ...
    //   decreases(a.length - startIndex)
      ...
      //     }
      //     check(seekEntryOrOpenDecoupled(k)(newArray, mask) == (i, 0))
      //     check(arrayForallSeekEntryOrOpenFound(startIndex)(a.updated(i, k), mask)) //TODO
      //   } else {
      ...
      //         lemmaPutValidKeyPreservesForallSeekEntryOrOpenStartIndex(a, i, k, startIndex + 1)
      //       }
      //       check(arrayForallSeekEntryOrOpenFound(startIndex)(a.updated(i, k), mask))
            
      //       } else {
      //         if(startIndex < a.length - 1){
      //           lemmaPutValidKeyPreservesForallSeekEntryOrOpenStartIndex(a, i, k, startIndex + 1)
      //         }
      //         check(arrayForallSeekEntryOrOpenFound(startIndex)(a.updated(i, k), mask) == arrayForallSeekEntryOrOpenFound(startIndex + 1)(a.updated(i, k), mask))
      //         check(arrayForallSeekEntryOrOpenFound(startIndex)(a.updated(i, k), mask))
    ...
    def lemmaSeekEntryOrOpenFindsThenSeekEntryFinds(k: Long, i: Int, a: Array[Long], mask: Int): Unit = {
      ...
      require(arrayForallSeekEntryOrOpenFound(0)(a, mask))
      require(validKeyInArray(k))
      require(seekEntryOrOpenDecoupled(k)(a, mask) == ResOk(i))
