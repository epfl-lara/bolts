...
object ConcSeqObj:
  import BalanceConcObj.Conc
  import BalanceConcObj.max

  import BalanceConcArrCharObj.ConcChar

  sealed trait ConcSeq[T] {
    ...
    @ghost def dropList(n: BigInt): List[T]

    def ++(ys: ConcSeq[T]): ConcSeq[T] = { 
      require(isBalanceConc(this) && isBalanceConc(ys) || isBalanceConcArrChar(this) && isBalanceConcArrChar(ys))
      ??? : ConcSeq[T]
    }.ensuring(res => res.list == this.list ++ ys.list)

    def slice(from: BigInt, until: BigInt): ConcSeq[T] = {
      require(0 <= from && from <= until && until <= this.size)
      ??? : ConcSeq[T]
    }.ensuring(res => res.list == this.list.slice(from, until))

    def splitAt(i: BigInt): (ConcSeq[T], ConcSeq[T]) = {
      require(0 <= i && i <= this.size)
      ??? : (ConcSeq[T], ConcSeq[T])
    }.ensuring(res => (res._1.list, res._2.list) == this.list.splitAtIndex(i))

    def filter(p: T => Boolean): ConcSeq[T] = {
      ??? : ConcSeq[T]
    }.ensuring(res => res.list == this.list.filter(p))

    def append(v: T): ConcSeq[T] = {
          ??? :  ConcSeq[T]
    }.ensuring(res => res.list == (this.list :+ v))
    def prepend(v: T): ConcSeq[T] = {
          ??? : ConcSeq[T]
    }.ensuring(res => res.list == (Cons(v, this.list)))
    def tail: ConcSeq[T] = {
      require(!this.isEmpty)
      ??? : ConcSeq[T]
    }.ensuring(res => res.list == this.list.tail)
  }

  inline def isBalanceConc[T](x: ConcSeq[T]): Boolean = {
    x match
     case BalanceConc(_) => true
     case _ => false
  }

  inline def isBalanceConcArrChar[T](x: ConcSeq[T]): Boolean = {
    x match
     case BalanceConcArrChar(_) => true
     case _ => false
  }

  case class BalanceConcArrChar(c: ConcChar) extends ConcSeq[Char] {
    require(c.isBalanced)

    @ghost override def list: List[Char] = c.list
    
    override def efficientList: List[Char] = {
      c.efficientList()
    }.ensuring(res => list == this.list)
    ...
    override def contains(v: Char): Boolean = {
      c.contains(v)
    }.ensuring(res => res == this.list.contains(v))
    ...
    @ghost def dropList(n: BigInt): List[Char] = c.dropList(n)
    
    override def ++(ys: ConcSeq[Char]): ConcSeq[Char] = { 
      require(isBalanceConcArrChar(ys))
      require(ys.asInstanceOf[BalanceConcArrChar].isBalanced && this.c.isBalanced)
      assert(c.isBalanced && ys.asInstanceOf[BalanceConcArrChar].c.isBalanced)
      BalanceConcArrChar(c ++ ys.asInstanceOf[BalanceConcArrChar].c)
    }.ensuring(res => 
      BalanceConcArrCharObj.appendAssocInst(this.c, ys.asInstanceOf[BalanceConcArrChar].c) &&
      res.asInstanceOf[BalanceConcArrChar].isBalanced &&
      res.asInstanceOf[BalanceConcArrChar].height <= max(this.height, ys.asInstanceOf[BalanceConcArrChar].height) + 1 &&
      res.asInstanceOf[BalanceConcArrChar].height >= max(this.height, ys.asInstanceOf[BalanceConcArrChar].height) &&
      res.list == this.list ++ ys.list)

    override def slice(from: BigInt, until: BigInt): BalanceConcArrChar = {
      require(0 <= from && from <= until && until <= this.size && this.isBalanced)
      BalanceConcArrChar(c.slice(from, until))
    }.ensuring(res => res.isBalanced && res.list == this.list.slice(from, until))

    
    override def splitAt(i: BigInt): (BalanceConcArrChar, BalanceConcArrChar) = {
      require(this.isBalanced)
      require(0 <= i && i <= this.size)
      val split = c.splitAt(i)
      (BalanceConcArrChar(split._1), BalanceConcArrChar(split._2))
    }.ensuring(res => (res._1.isBalanced && res._2.isBalanced) && (res._1.list, res._2.list) == this.list.splitAtIndex(i))

    
    override def filter(p: Char => Boolean): BalanceConcArrChar = {
      BalanceConcArrChar(c.filter(p))
    }.ensuring(res => res.isBalanced && res.list == this.list.filter(p))

    
    override def append(v: Char): BalanceConcArrChar = {
      BalanceConcArrChar(c.append(v))
    }.ensuring(res => res.isBalanced && res.list == (this.list :+ v))
    
    override def prepend(v: Char): BalanceConcArrChar = {
      BalanceConcArrChar(c.prepend(v))
    }.ensuring(res => res.isBalanced && res.list == (Cons(v, this.list)))
    
    override def tail: BalanceConcArrChar = {
      require(!this.isEmpty)
      BalanceConcArrChar(c.tail)
    }.ensuring(res => res.isBalanced && res.list == this.list.tail)

  }
  
  case class BalanceConc[T](c: Conc[T]) extends ConcSeq[T] {
    require(c.isBalanced)

    @ghost override def list: List[T] = c.list
    
    override def efficientList: List[T] = {
      c.efficientList()
    }.ensuring(res => list == this.list)
    ...
    override def ++(ys: ConcSeq[T]): ConcSeq[T] = { 
      require(isBalanceConc(ys))
      require(ys.asInstanceOf[BalanceConc[T]].isBalanced && this.c.isBalanced)
      assert(c.isBalanced && ys.asInstanceOf[BalanceConc[T]].c.isBalanced)
      BalanceConc(c ++ ys.asInstanceOf[BalanceConc[T]].c)
    }.ensuring(res => 
      BalanceConcObj.appendAssocInst(this.c, ys.asInstanceOf[BalanceConc[T]].c) &&
      res.asInstanceOf[BalanceConc[T]].isBalanced &&
      res.asInstanceOf[BalanceConc[T]].height <= max(this.height, ys.asInstanceOf[BalanceConc[T]].height) + 1 &&
      res.asInstanceOf[BalanceConc[T]].height >= max(this.height, ys.asInstanceOf[BalanceConc[T]].height) &&
      res.list == this.list ++ ys.list)
    ...
    override def slice(from: BigInt, until: BigInt): BalanceConc[T] = {
      require(0 <= from && from <= until && until <= this.size && this.isBalanced)
      BalanceConc(c.slice(from, until))
    }.ensuring(res => res.isBalanced && res.list == this.list.slice(from, until))

    
    override def splitAt(i: BigInt): (BalanceConc[T], BalanceConc[T]) = {
      require(this.isBalanced)
      require(0 <= i && i <= this.size)
      val split = c.splitAt(i)
    ...
    override def filter(p: T => Boolean): BalanceConc[T] = {
      BalanceConc(c.filter(p))
    }.ensuring(res => res.isBalanced && res.list == this.list.filter(p))

    
    override def append(v: T): BalanceConc[T] = {
      BalanceConc(c.append(v))
    }.ensuring(res => res.isBalanced && res.list == (this.list :+ v))
    
    override def prepend(v: T): BalanceConc[T] = {
      BalanceConc(c.prepend(v))
    }.ensuring(res => res.isBalanced && res.list == (Cons(v, this.list)))
    
    override def tail: BalanceConc[T] = {
      require(!this.isEmpty)
      BalanceConc(c.tail)
    }.ensuring(res => res.isBalanced && res.list == this.list.tail)
  ...
  def emptyB[T]: BalanceConc[T] = BalanceConc(BalanceConcObj.Empty())
  @pure @opaque 
  def singletonB[T](v: T): BalanceConc[T] = BalanceConc(BalanceConcObj.Leaf(v))
  def fromListB[T](l: List[T]): BalanceConc[T] = {
    BalanceConc(BalanceConcObj.fromList(l))
  ...
  def appendAssocInstB[T](xs: BalanceConc[T], ys: BalanceConc[T]): Boolean = {
    BalanceConcObj.appendAssocInst(xs.c, ys.c)
  }.holds
...
object BalanceConcArrCharObj:
  ...
  extension(t: ConcChar)
    ...
    @pure @ghost @inlineOnce
    def dropList(n: BigInt): List[Char] = {
      t.list.drop(n)
    }
