...
object MemoisationZipper {
  ...
  @ghost def validCacheMapFindLongestMatch[C](m: HashMap[(Zipper[C], BigInt), BigInt], totalInput: Sequence[C]): Boolean = {
    m.valid && 
    TupleListOpsGenK.invariantList(m.map.toList) && // Why is this needed? Without it does not verify in update...
...
object ZipperRegex {
  ...
  @opaque
  @inlineOnce
  def findLongestMatchZipperFastV2MemOnlyDerivTailRec[C](z: Zipper[C], input: Sequence[C], totalInput: Sequence[C])(using cacheUp: CacheUp[C], cacheDown: CacheDown[C]): (Sequence[C], Sequence[C]) = {
    require(ListUtils.isSuffix(input.list, totalInput.list))
    require(cacheUp.valid)
    require(cacheDown.valid)
    val totalInputSize = totalInput.size
    val from = totalInputSize - input.size
    val prefixLength = findLongestMatchInnerZipperFastV2MemTailRecWrapper(z, from, totalInput, totalInputSize)

    ghostExpr(unfold(findLongestMatchInnerZipperFastV2MemTailRecWrapper(z, from, totalInput, totalInputSize)))
    ghostExpr(unfold(findLongestMatchZipperFastV2MemOnlyDeriv(z, input, totalInput)))
    ghostExpr(unfold(findLongestMatchZipperFastV2(z, input, totalInput)))
  ...
  /**
   * Tail-recursive version of findLongestMatchInnerZipperFastV2Mem.
   * Instead of recursing and then adding 1 to the result, we iterate forward and track
   * the last position where the zipper was nullable.
   * 
   * @param z The current zipper state
   * @param from The starting position in totalInput
   * @param currentPos The current position we're examining
   * @param totalInput The full input sequence
   * @param totalInputSize Size of totalInput (passed to avoid recomputation)
   * @param lastNullableOffset The offset from `from` of the last nullable zipper found (0 means none found after start)
   */
  @tailrec
  def findLongestMatchInnerZipperFastV2MemTailRec[C](
    z: Zipper[C], 
    from: BigInt, 
    currentPos: BigInt, 
    totalInput: Sequence[C], 
    totalInputSize: BigInt,
    lastNullableOffset: BigInt
  )(using cacheUp: CacheUp[C], cacheDown: CacheDown[C]): BigInt = {
    require(from >= 0 && from <= totalInput.size)
    require(currentPos >= from && currentPos <= totalInputSize)
    require(totalInputSize == totalInput.size)
    require(lastNullableOffset >= 0 && lastNullableOffset <= currentPos - from)
    decreases(totalInputSize - currentPos)

    if ((currentPos == totalInputSize) || lostCauseZipper(z)) {
      // We've reached the end or a lost cause, return the last nullable offset
      lastNullableOffset
    } else {
      val derivedZ = derivationStepZipperMem(z, totalInput(currentPos))
      val newOffset = currentPos - from + 1
      val newLastNullableOffset = if nullableZipper(derivedZ) then newOffset else lastNullableOffset
      findLongestMatchInnerZipperFastV2MemTailRec(derivedZ, from, currentPos + 1, totalInput, totalInputSize, newLastNullableOffset)
    }
  }.ensuring(res => res >= 0 && res <= (totalInputSize - from) && cacheUp.valid && cacheDown.valid)

  /**
   * Lemma proving that the tail-recursive version is equivalent to the original recursive version.
   */
  @ghost
  @opaque
  @inlineOnce
  def lemmaFindLongestMatchTailRecEquivRec[C](
    z: Zipper[C], 
    from: BigInt, 
    currentPos: BigInt, 
    totalInput: Sequence[C], 
    totalInputSize: BigInt,
    lastNullableOffset: BigInt
  )(using cacheUp: CacheUp[C], cacheDown: CacheDown[C]): Unit = {
    require(from >= 0 && from <= totalInput.size)
    require(currentPos >= from && currentPos <= totalInputSize)
    require(totalInputSize == totalInput.size)
    require(lastNullableOffset >= 0 && lastNullableOffset <= currentPos - from)
    require(cacheUp.valid)
    require(cacheDown.valid)
    // Key invariant: lastNullableOffset captures the longest match found so far from `from` to `currentPos-1`
    require({
      val recResult = findLongestMatchInnerZipperFastV2(z, currentPos, totalInput, totalInputSize)
      // If recResult > 0, then the total match from `from` is max(lastNullableOffset, currentPos - from + recResult)
      // If recResult == 0, then the total match from `from` is lastNullableOffset
      true
    })
    decreases(totalInputSize - currentPos)

    if ((currentPos == totalInputSize) || lostCauseZipper(z)) {
      // Base case: at the end or lost cause
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2(z, currentPos, totalInput, totalInputSize)))
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2MemTailRec(z, from, currentPos, totalInput, totalInputSize, lastNullableOffset)))
      // When we hit end/lostCause, recursive version returns 0
      // Tail-rec returns lastNullableOffset
      ()
    } else {
      val derivedZ = derivationStepZipper(z, totalInput(currentPos))
      val newOffset = currentPos - from + 1
      val newLastNullableOffset = if nullableZipper(derivedZ) then newOffset else lastNullableOffset
      ...
      // Recursive call
      lemmaFindLongestMatchTailRecEquivRec(derivedZ, from, currentPos + 1, totalInput, totalInputSize, newLastNullableOffset)
    }
  }.ensuring(_ => cacheUp.valid && cacheDown.valid)

  /**
   * Main wrapper that calls the tail-recursive version and proves equivalence.
   */
  @opaque
  @inlineOnce
  def findLongestMatchInnerZipperFastV2MemTailRecWrapper[C](
    z: Zipper[C], 
    from: BigInt, 
    totalInput: Sequence[C], 
    totalInputSize: BigInt
    ...
    val result = findLongestMatchInnerZipperFastV2MemTailRec(z, from, from, totalInput, totalInputSize, BigInt(0))
    
    ghostExpr({
      lemmaFindLongestMatchTailRecEquivRecMain(z, from, totalInput, totalInputSize)
    })
  ...
  /**
   * Main equivalence lemma: proves that starting the tail-recursive version from position `from` 
   * with lastNullableOffset=0 gives the same result as the original recursive version.
   */
  @ghost
  @opaque
  @inlineOnce
  def lemmaFindLongestMatchTailRecEquivRecMain[C](
    z: Zipper[C], 
    from: BigInt, 
    totalInput: Sequence[C], 
    totalInputSize: BigInt
    ...
    if ((from == totalInputSize) || lostCauseZipper(z)) {
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2(z, from, totalInput, totalInputSize)))
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2MemTailRec(z, from, from, totalInput, totalInputSize, BigInt(0))))
    } else {
      val derivedZ = derivationStepZipper(z, totalInput(from))
      val recResult = findLongestMatchInnerZipperFastV2(derivedZ, from + 1, totalInput, totalInputSize)
      
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2(z, from, totalInput, totalInputSize)))
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2MemTailRec(z, from, from, totalInput, totalInputSize, BigInt(0))))
      
      val newLastNullableOffset = if nullableZipper(derivedZ) then BigInt(1) else BigInt(0)
      
      // We need to prove that:
      // findLongestMatchInnerZipperFastV2MemTailRec(derivedZ, from, from + 1, totalInput, totalInputSize, newLastNullableOffset)
      // equals the result computed by the recursive version
      lemmaFindLongestMatchTailRecEquivRecInner(derivedZ, from, from + 1, totalInput, totalInputSize, newLastNullableOffset)
    }
  }.ensuring(_ => findLongestMatchInnerZipperFastV2MemTailRec(z, from, from, totalInput, totalInputSize, BigInt(0)) == 
                  findLongestMatchInnerZipperFastV2(z, from, totalInput, totalInputSize) && 
                  cacheUp.valid && cacheDown.valid)

  /**
   * Inner lemma that relates the tail-recursive computation to the recursive one.
   * 
   * Key insight: At position currentPos with lastNullableOffset accumulated,
   * the tail-rec result equals max(lastNullableOffset, currentPos - from + recResult) when recResult > 0,
   * or lastNullableOffset when recResult == 0.
   */
  @ghost
  @opaque
  @inlineOnce
  def lemmaFindLongestMatchTailRecEquivRecInner[C](
    z: Zipper[C], 
    from: BigInt, 
    currentPos: BigInt, 
    totalInput: Sequence[C], 
    ...
    val recResult = findLongestMatchInnerZipperFastV2(z, currentPos, totalInput, totalInputSize)
    val tailRecResult = findLongestMatchInnerZipperFastV2MemTailRec(z, from, currentPos, totalInput, totalInputSize, lastNullableOffset)
    
    if ((currentPos == totalInputSize) || lostCauseZipper(z)) {
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2(z, currentPos, totalInput, totalInputSize)))
      ghostExpr(unfold(findLongestMatchInnerZipperFastV2MemTailRec(z, from, currentPos, totalInput, totalInputSize, lastNullableOffset)))
      // recResult == 0, tailRecResult == lastNullableOffset
      // The expected combined result is lastNullableOffset
      check(tailRecResult == lastNullableOffset)
      check(recResult == BigInt(0))
    } else {
      val derivedZ = derivationStepZipper(z, totalInput(currentPos))
      val newOffset = currentPos - from + 1
      val newLastNullableOffset = if nullableZipper(derivedZ) then newOffset else lastNullableOffset
      ...
      lemmaFindLongestMatchTailRecEquivRecInner(derivedZ, from, currentPos + 1, totalInput, totalInputSize, newLastNullableOffset)
      
      val innerRecResult = findLongestMatchInnerZipperFastV2(derivedZ, currentPos + 1, totalInput, totalInputSize)
      
      // The recursive result for current position:
      // if innerRecResult > 0: 1 + innerRecResult
      // else: if nullableZipper(derivedZ) then 1 else 0
      
      // The tail-rec invariant should maintain that tailRecResult captures the max match length from `from`
    }
  }.ensuring(_ => {
    val recResult = findLongestMatchInnerZipperFastV2(z, currentPos, totalInput, totalInputSize)
    val tailRecResult = findLongestMatchInnerZipperFastV2MemTailRec(z, from, currentPos, totalInput, totalInputSize, lastNullableOffset)
    // The invariant: tailRecResult == max(lastNullableOffset, currentPos - from + recResult) when measured from `from`
    // But since recResult measures from currentPos, the total from `from` is (currentPos - from) + recResult
    // Actually, we need: tailRecResult == max(lastNullableOffset, recResult + (currentPos - from)) only when recResult > 0
    // When recResult == 0, tailRecResult == lastNullableOffset
    (if recResult > 0 then tailRecResult == recResult + (currentPos - from) else tailRecResult == lastNullableOffset) &&
    cacheUp.valid && cacheDown.valid
  })

  // ------------------------------------------- END TAIL-RECURSIVE VERSION -------------------------------------------
